...
public class MafhamMoonsOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
            ...
            if (npc.getId() == frozenWeaponsNPCID) {
                if (npc.getAnimation() == frozenWeaponsAnimID || npc.getAnimation() == frozenWeaponsAnimID2) {
                    ...
                    if (localPoint != null) {
                        ...
                        renderClickbox(graphics, clickbox, client.getMouseCanvasPosition(), color, color, 255, 20,
                                color, true);
                    }
                }
            }
        ...
        if (plugin.getSpecTimer() != null && plugin.getSpecTimer() > -1) {
            ...
            if (player != null) {
                Point point = player.getCanvasTextLocation(graphics, String.valueOf(plugin.getSpecTimer()),
    ...
    public static void renderClickbox(Graphics2D graphics, Shape area, Point mousePosition, Color line, Color fill,
            int lineAlpha, int fillAlpha, Color hovered, boolean antiAlias) {
        if (area != null) {
    ...
    private void renderTextLocation(Graphics2D graphics, String txtString, int fontSize, int fontStyle, Color fontColor,
    ...
    private void renderPoly(Graphics2D graphics, Color outlineColor, Color fillColor, int lineAlpha, int fillAlpha,
            Shape polygon, double width, boolean antiAlias) {
        if (polygon != null) {
            graphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
                    antiAlias ? RenderingHints.VALUE_ANTIALIAS_ON : RenderingHints.VALUE_ANTIALIAS_OFF);
            graphics.setColor(
                    new Color(outlineColor.getRed(), outlineColor.getGreen(), outlineColor.getBlue(), lineAlpha));
