...
class NexOverlay extends Overlay
{
	...
	public Dimension render(Graphics2D graphics)
	{
		...
		if (plugin.getCurrentPhase() == NexPhase.ZAROS)
		{
			Color color = Color.white;
			if (plugin.getZarosAttackCounter() == 0)
			{
				color = Color.GREEN;
			}
			String string = String.valueOf(plugin.getZarosAttackCounter());
			Point canvasPoint = plugin.getNex().getCanvasTextLocation(graphics, string, 240);
			renderTextLocation(graphics, string, 24, 1, color, canvasPoint);
		}
		...
		{
			int id = npc.getId();
			if (id == 11286 || id == 11285 || id == 11284 || id == 11283) //minions
			{
				if (npc.getInteracting() != null)
				{
					if (npc.getInteracting() instanceof Player)
					{
						String string = npc.getInteracting().getName();
						Point canvasPoint = npc.getCanvasTextLocation(graphics, string, 150);
						renderTextLocation(graphics, string, 14, 0, Color.CYAN, canvasPoint);
					}
				}
			}
		}
