...
public class MafhamColoOverlay extends Overlay {
    ...
    @Inject
    private MafhamColoOverlay(MafhamColoPlugin plugin, Client client, ModelOutlineRenderer modelOutlineRenderer,
            NpcUtil npcUtil, MafhamColoConfig config) {
        this.client = client;
        this.plugin = plugin;
    ...
    private void renderTextLocation(Graphics2D graphics, String txtString, int fontSize, int fontStyle, Color fontColor,
            net.runelite.api.Point canvasPoint, Boolean shadow) {
    ...
    private boolean hasLineOfSight(WorldArea playerWorldArea, WorldArea npcWorldArea) {
        return playerWorldArea.hasLineOfSightTo(client.getTopLevelWorldView(), npcWorldArea);
    }
